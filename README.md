...
Walkthrough videos (mp4, open the link or clone the repo):

- [RKE2 cluster](docs/media/video-preview-rke2.mp4) (42 MB)
- [kubeadm cluster](docs/media/video-preview-kubeadm.mp4) (28 MB)
...
| 8 | Helm | releases from `sh.helm.release.v1` secrets, values, history, update check; `u` upgrade, `b` rollback |
...
- no Go: `./build.sh linux amd64` (Docker) -> `dist/khealth-linux-amd64`; also `windows amd64`, `darwin arm64`
- Go 1.26+: `go build -o khealth ./cmd/khealth`, or `make build` / `make test`
