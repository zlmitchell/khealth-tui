...
| 5 | Storage | StorageClasses, CSI drivers (per-node registration), PVCs with **used capacity** (kubelet `stats/summary`) and the **backend's own health** (Longhorn robustness, healthy/wanted replicas), PVs, node filesystems |
...
* rke2 upgrade readiness - kubelet version skew across nodes and `rancher-system-agent` plan failures/applications are checked; `system-upgrade-controller` plans and pending Rancher plans are not read
* CSI backend health - Longhorn (volumes, replicas, nodes, disks, backup target, orphans, settings) and Trident (backends, backend configs, orchestrator, nodes, publications) via their CRDs, VolumeAttachments for every driver, node-side stale devices and hung mounts; other drivers (Ceph/Rook, Portworx, vSphere CNS) still only get the generic controller/node-plugin/attachment checks
...
* `system-upgrade-controller` plans, pending Rancher plans
