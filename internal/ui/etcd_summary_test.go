...
func TestEtcdSummaryFallsBackToExec(t *testing.T) {
	...
	a.etcdExec = nil
	if s := a.etcdSummarize(); s.probed != 0 || s.noProbeReason != "no etcd nodes probed" {
	...
	if s := a.etcdSummarize(); s.noProbeReason != "1 probe error" {
		t.Errorf("errored probe: reason=%q", s.noProbeReason)
	}
	a.etcdExec = &etcd.Probe{Err: errors.New("pods/exec forbidden")}
	if s := a.etcdSummarize(); s.noProbeReason != "1 probe error, exec failed" {
