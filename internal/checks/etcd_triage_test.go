...
func TestTriageClusterDownPicksLastLeader(t *testing.T) {
	...
	for _, n := range []string{"cp-1", "cp-2", "cp-3"} {
		in.Nodes[n] = &nodeinfo.Info{Node: n, Services: []nodeinfo.Service{{Name: "rke2-server", Active: "active", Sub: "running"}}}
	}
	...
	for _, want := range []string{
		...
		"on EVERY other server first: systemctl stop rke2-server",
	...
	order := []string{"1. cp-3", "on EVERY other server first: systemctl stop", "rke2 server --cluster-reset", "systemctl start rke2-server on cp-3", "rm -rf /var/lib/rancher/rke2/server/db"}
