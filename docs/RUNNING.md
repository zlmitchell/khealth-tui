...
`khealth --init-config` writes the annotated example config to `~/.config/k8s-health-tui/config.yaml` (`%AppData%/k8s-health-tui/config.yaml` on Windows; `--config <path>` to put it elsewhere; it never overwrites) and `--print-config` prints it to stdout. `./k8s-health-tui.yaml` and `./khealth.yaml` in the working directory are also picked up. Every key is optional; flags override the file. The source of the example is `internal/config/config.example.yaml`.
