...
| **etcd via `kubectl exec`** | etcd pods | every `refresh` tick | `etcdctl member list`, `endpoint health/status`, `alarm list` inside the pod, plus one Secret sampled from etcd (first 24 bytes: the `k8s:enc:` prefix proves encryption at rest for STIG V-274882) |
