...
| **Cloud / CSI** (part of the snapshot) | kube-apiserver | every tick | `volumeattachments` list; `tridentbackends.trident.netapp.io` list (skipped for `perf.denied_ttl` when the CRD is absent) and, only when it exists, `tridentorchestrators`, `tridentbackendconfigs`, `tridentnodes`, `tridentvolumepublications`; `volumes.longhorn.io` list and, only when it exists, `replicas`, `engines`, `sharemanagers`, `instancemanagers`, `nodes`, `engineimages`, `backuptargets`, `orphans`, `settings` (CRs are JSON, no protobuf: budget ~4 KB per volume, ~2.5 KB per replica, ~5 KB+ per engine - its snapshot tree grows with the snapshots); one GET of `kube-system/vsphere-cloud-config`; the rest (`Snapshot.Cloud`) is derived from lists already taken |
